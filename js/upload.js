var Upload = {
  image: function(inputElement, callback) {
    ...
    if (!this.validateImageSize(file, 5)) { alert('图片大小不能超过 5MB'); return; }
  ...
  images: function(inputElement, callback, maxCount) {
    ...
    for (var i = 0; i < max; i++) {
      ...
      if (!this.validateImageSize(file, 3)) continue;
      (function(index, file) {
        var reader = new FileReader();
        reader.onload = function(e) {
          results.push(e.target.result);
          loaded++;
          if (loaded === max || i === max - 1) callback(results);
        };
        reader.readAsDataURL(file);
      })(i, file);
    }
  ...
  pdf: function(inputElement, callback) {
    ...
    if (file.size > 2 * 1024 * 1024) { alert('PDF 大小不能超过 2MB'); return; }
    var reader = new FileReader();
    reader.onload = function(e) { callback(e.target.result, file.name); };
    reader.onerror = function() { alert('PDF 读取失败，请重试'); };
    reader.readAsDataURL(file);
  }
};
